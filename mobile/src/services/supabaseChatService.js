...
const supabaseUrl = process.env.EXPO_PUBLIC_SUPABASE_URL || 'https://ixclqlsrvimlrfddiror.supabase.co';
const supabaseAnonKey = process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY || 'sb_publishable_-33Ckq_otXTInQB66lJrAA_kbLt7slA';
...
export const listenToMessages = (chatId, callback, onError) => {
  ...
  const channel = supabase
    .channel(`public:chat_messages:chat_id=eq.${chatId}_${Date.now()}`)
...
export const listenToConversationSummaries = (userId, callback, onError) => {
  ...
  const channelParts = supabase
    .channel(`public:chat_participants:user_id=eq.${ownerId}_${Date.now()}`)
  ...
  const channelConvs = supabase
    .channel(`public:chat_conversations_${Date.now()}`)
