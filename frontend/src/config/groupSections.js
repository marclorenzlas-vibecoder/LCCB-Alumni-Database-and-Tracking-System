// Shared group/level definitions used by AlumniDirectory and Register
...
export const groupSectionDefinitions = [
  {
  ...
  },
  {
    ...
    gridClassName: 'grid gap-2 sm:grid-cols-2',
    items: [
      { value: 'B.A. in English Language Studies', label: 'Bachelor of Arts — English Language Studies', description: 'Bachelor of Arts in English Language Studies (ABELS)' },
      { value: 'B.S. in Business Administration', label: 'Bachelor of Science — Business Administration', description: 'Business Administration (BSBA) program' },
      { value: 'B.S. in Hospitality Management', label: 'Bachelor of Science — Hospitality Management', description: 'Hospitality Management (BSHM) program' }
    ]
  },
  {
    key: 'GRAD_SCHOOL',
...
// Helper: flattened list of courses for simple selects
export const flattenedCourseOptions = groupSectionDefinitions.reduce((acc, sec) => {
  sec.items.forEach(it => acc.push({ value: it.value, label: it.label }));
  return acc;
}, []);
