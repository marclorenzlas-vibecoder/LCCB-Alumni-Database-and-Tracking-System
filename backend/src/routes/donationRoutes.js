...
router.post('/:id/contribute', flexibleAuthMiddleware, async (req, res) => {
  try {
    ...
    const updatedDescription = buildDescriptionWithMeta(
      [existingData.cleanDescription, cleanDescription].filter(Boolean).join('\n\n'),
      mergedMeta
    );
    ...
    try {
      ...
      const donationMode = String(meta.donationMode || '').toLowerCase();
      const donationCurrency = String(meta.paymentCurrency || 'PHP').toUpperCase();
      const donationKind = donationMode === 'both'
        ? 'money and items'
        : donationMode === 'items' || donationMode === 'item' || donationMode === 'goods'
          ? 'items'
          : 'money';
      const amountLabel = contributionAmount > 0 ? formatDonationAmount(contributionAmount, donationCurrency) : 'an item donation';
